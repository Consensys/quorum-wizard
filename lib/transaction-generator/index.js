...
function sendTransaction(name) {
  const index = (contracts[name].sendFromOwner) ? contracts[name].owner : getRandomInt(defaultAccounts.length)
  const web3 = web3s[index]
  const account = defaultAccounts[index]
  const contract = new web3.eth.Contract(contracts[name].abi, contracts[name].contract.options.address);
  console.log(`(${name}) sending from (${index}) ${account} to ${contracts[name].contract.options.address}`)
  let val = contractMethods[name].value
  if (contractMethods[name].method === 'transfer') {
    let to = defaultAccounts[getRandomInt(defaultAccounts.length)]
    while (to === account) {
      to = defaultAccounts[getRandomInt(defaultAccounts.length)]
    }
    val[0] = to
  }
  contract.methods[contractMethods[name].method](...val)
    .send({ from: account })
